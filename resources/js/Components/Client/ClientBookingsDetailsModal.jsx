...
export default function ClientBookingsDetailsModal({ showBookingDetails, setShowBookingDetails, formatDate, formatTime, calculateDaysFromNow }) {
  // Debug: Log the booking details to see what data we're working with
  console.log('Booking Details:', showBookingDetails);
                ...
                <div className="flex items-center">
                  ...
                  <div>
                    ...
                    <p className="font-medium text-gray-900">
                      {/* Direct access to roomNumber field from database */}
                      {showBookingDetails.roomNumber ? `Room ${showBookingDetails.roomNumber}` : 'Room N/A'}
                    </p>
                  </div>
                </div>
                <div className="flex items-center">
                  ...
                  <div>
                    ...
                    <p className="font-medium text-gray-900">
                      {/* Direct access to roomType field from database with proper capitalization */}
                      {showBookingDetails.roomType 
                        ? showBookingDetails.roomType.charAt(0).toUpperCase() + showBookingDetails.roomType.slice(1) 
                        : 'Standard'}
                    </p>
                  </div>
                </div>
          ...
          <div className="mb-6 rounded-lg border border-[#E8DCCA] bg-white p-4 shadow-sm">
            ...
            <div className="bg-[#F5EFE7]/30 rounded-lg p-3 border border-[#E8DCCA]">
              <div className="flex flex-wrap gap-2">
                {(() => {
                  // Default amenities that should always be available
                  const defaultAmenities = ['Free Wi-Fi', 'Breakfast', 'Air Conditioning', 'Room Service'];
                  
                  // Get amenities from the booking object, handling different data structures
                  let amenitiesList = [];
                  
                  // Try to get amenities from the booking data
                  if (Array.isArray(showBookingDetails.amenities) && showBookingDetails.amenities.length > 0) {
                    amenitiesList = [...showBookingDetails.amenities];
                  } else if (showBookingDetails.room?.amenities) {
                    try {
                      // Try to parse if it's a JSON string
                      if (typeof showBookingDetails.room.amenities === 'string') {
                        const parsedAmenities = JSON.parse(showBookingDetails.room.amenities);
                        if (Array.isArray(parsedAmenities) && parsedAmenities.length > 0) {
                          amenitiesList = [...parsedAmenities];
                        }
                      } else if (Array.isArray(showBookingDetails.room.amenities) && showBookingDetails.room.amenities.length > 0) {
                        amenitiesList = [...showBookingDetails.room.amenities];
                      }
                    } catch (e) {
                  ...
                  // If no amenities were found, use the default list
                  if (amenitiesList.length === 0) {
                    amenitiesList = defaultAmenities;
                  }
                  
                  // Format and display the amenities
                  return amenitiesList.map((amenity, index) => {
                    // Skip null or undefined amenities
                    if (!amenity) return null;
                    
                    // Format the amenity text
                    const amenityText = typeof amenity === 'string' ? amenity : String(amenity);
                    // Format amenity text to be properly capitalized
                    const formattedAmenity = amenityText
                      .split('_')
                    ...
                    // Select the appropriate icon based on the amenity
                    if (amenityLower.includes('wifi')) icon = <Wifi className="h-3.5 w-3.5" />;
                    else if (amenityLower.includes('breakfast')) icon = <Coffee className="h-3.5 w-3.5" />;
                    else if (amenityLower.includes('pool')) icon = <Bath className="h-3.5 w-3.5" />;
                    else if (amenityLower.includes('spa')) icon = <Sparkles className="h-3.5 w-3.5" />;
                    else if (amenityLower.includes('bar')) icon = <Wine className="h-3.5 w-3.5" />;
                    else if (amenityLower.includes('parking')) icon = <Car className="h-3.5 w-3.5" />;
                    else if (amenityLower.includes('restaurant')) icon = <Utensils className="h-3.5 w-3.5" />;
                    else if (amenityLower.includes('air') || amenityLower.includes('conditioning')) icon = <Wind className="h-3.5 w-3.5" />;
                    else if (amenityLower.includes('room service')) icon = <BellRing className="h-3.5 w-3.5" />;
                    
                    return (
                      <span key={index} className="rounded-full bg-[#A67C52]/10 px-3 py-1.5 text-xs font-medium text-[#8B5A2B]">
                        <div className="flex items-center">
                          {icon}
                          <span className="ml-1.5">{formattedAmenity}</span>
                        </div>
                      </span>
                    );
                  });
                })()} {/* Added the missing () to execute the IIFE */}
              </div>
            </div>
          </div>
