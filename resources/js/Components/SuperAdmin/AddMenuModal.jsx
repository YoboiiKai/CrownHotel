...
export default function AddMenuModal({ show, onClose, onSubmit }) {
  ...
  const validateForm = () => {
    ...
    if (!formData.menuname.trim()) {
      newErrors.menuname = "Item name is required"
    }
    ...
    if (!formData.preperationtime.trim()) {
  ...
  const handleSubmit = (e) => {
    ...
    if (Object.keys(newErrors).length > 0) {
      ...
      axios.post('/api/menu', formDataToSend, {
        headers: {
        ...
        .catch(error => {
          if (error.response && error.response.data && error.response.data.errors) {
            ...
            setErrors({ general: "An error occurred while creating the menu item" })
