...
export default function AddInventoryModal({ show, onClose, onSubmit }) {
  ...
  const handleSubmit = (e) => {
    ...
    if (Object.keys(newErrors).length > 0) {
      ...
      axios.post('/api/inventory', formDataToSend, {
        headers: {
        ...
        .catch(error => {
          if (error.response && error.response.data && error.response.data.errors) {
            ...
            setErrors({ general: "An error occurred while creating the inventory item" })
                    ...
                    <form onSubmit={handleSubmit} className="space-y-6">
                        ...
                        <div className="flex items-center justify-end gap-3 pt-6 border-t border-gray-200 mt-8">
                            <button
                                ...
                                className="px-4 py-2.5 text-sm font-medium text-gray-700 bg-gray-100 rounded-lg shadow-sm hover:bg-gray-200 focus:outline-none focus:ring-2 focus:ring-gray-300 focus:ring-offset-1 transition-all"
                            >
                                Cancel
                            </button>
                            <button
