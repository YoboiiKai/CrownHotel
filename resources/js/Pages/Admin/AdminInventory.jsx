...
export default function AdminInventory({ auth }) {
  ...
  const getStockStatus = (item) => {
    if (item.quantity <= 0) {
      ...
      return { status: "low-stock", color: "bg-[#A67C52]/20 text-amber-800" };
    } else {
      ...
      <div className="mx-auto max-w-6xl">
        <div className="bg-white p-6 rounded-lg shadow-md overflow-auto">
          ...
          <div className="flex overflow-x-auto border-b border-gray-200 mb-6">
            <button
              className={`px-4 py-2 text-sm font-medium whitespace-nowrap ${filterCategory === "all" ? "text-[#8B5A2B] border-b-2 border-[#8B5A2B]" : "text-gray-500 group-hover:text-[#6B4226]"}`}
              onClick={() => setFilterCategory("all")}
            >
              All Categories
            </button>
            <button
              className={`px-4 py-2 text-sm font-medium whitespace-nowrap ${filterCategory === "food" ? "text-[#8B5A2B] border-b-2 border-[#8B5A2B]" : "text-gray-500 group-hover:text-[#6B4226]"}`}
              onClick={() => setFilterCategory("food")}
            >
              Food & Beverage
            </button>
            <button
              className={`px-4 py-2 text-sm font-medium whitespace-nowrap ${filterCategory === "housekeeping" ? "text-[#8B5A2B] border-b-2 border-[#8B5A2B]" : "text-gray-500 group-hover:text-[#6B4226]"}`}
              onClick={() => setFilterCategory("housekeeping")}
            >
              Housekeeping
            </button>
            <button
              className={`px-4 py-2 text-sm font-medium whitespace-nowrap ${filterCategory === "equipment" ? "text-[#8B5A2B] border-b-2 border-[#8B5A2B]" : "text-gray-500 group-hover:text-[#6B4226]"}`}
              onClick={() => setFilterCategory("equipment")}
            >
              Equipment
            </button>
            <button
              className={`px-4 py-2 text-sm font-medium whitespace-nowrap ${filterCategory === "amenities" ? "text-[#8B5A2B] border-b-2 border-[#8B5A2B]" : "text-gray-500 group-hover:text-[#6B4226]"}`}
                  ...
                  <div className="p-3">
                    ...
                    <div className="mb-3">
                      <div className="flex items-center justify-between mb-1">
                        ...
                        <div className="flex items-center gap-0.5">
                          ...
                          <span className="font-medium text-xs text-[#8B5A2B]">{item.price.toFixed(2)}</span>
                        </div>
                      </div>
                    ...
                    <div className="flex items-center gap-1 pt-2 border-t border-gray-100">
                      <button
                        ...
                        className="flex-1 flex items-center justify-center gap-1 rounded-lg bg-gradient-to-r from-[#A67C52] via-[#8B5A2B] to-[#6B4226] px-2 py-1.5 text-xs font-medium text-white shadow-sm hover:from-[#8B5A2B] hover:to-[#6B4226] focus:outline-none focus:ring-2 focus:ring-[#A67C52] focus:ring-offset-1 transition-all"
                      >
                      ...
                      <button
                        ...
                        className="flex-1 flex items-center justify-center gap-1 rounded-lg border border-[#A67C52]/30 bg-[#A67C52]/10 px-2 py-1.5 text-xs font-medium text-[#6B4226] hover:bg-[#A67C52]/20 focus:outline-none focus:ring-2 focus:ring-[#A67C52] focus:ring-offset-1 transition-all"
                      >
